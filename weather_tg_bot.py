...
async def help_commands(message: types.Message):
    await message.reply('I have time, random symbol, currency and information', reply_markup=main_menu)
...
async def get_weather(message:types.Message):
    ...
    if message.text == 'Data time':
        ...
        await bot.send_message(message.from_user.id, 'Developed by Nikita Shkurat')
    elif message.text == 'Random number':
        await bot.send_message(message.from_user.id, random.randint(0, 1000))
        ...
        try:
            r = requests.get(
                f'https://api.openweathermap.org/data/2.5/weather?q={message.text}&appid={open_weather_token}', # Your Token
                params={"units": "metric"})
            data = r.json()
